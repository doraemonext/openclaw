...
describe("Dockerfile", () => {
  it("uses the custom Ubuntu multi-stage layout with optional extension preinstall", async () => {
    const dockerfile = await readFile(dockerfilePath, "utf8");
    expect(dockerfile).toContain("FROM ubuntu:24.04 AS ubuntu-node");
    expect(dockerfile).toContain("FROM ubuntu-node AS ext-deps");
    expect(dockerfile).toContain("ARG OPENCLAW_EXTENSIONS");
    expect(dockerfile).toContain("COPY --from=ext-deps /out/ ./extensions/");
    expect(dockerfile).toContain("FROM ubuntu-node AS build");
    expect(dockerfile).toContain("FROM build AS runtime-assets");
    expect(dockerfile).toContain("FROM ubuntu-node");
  });
